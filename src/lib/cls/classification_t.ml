...
let () =
  ...
  add_simple_test ~title:"Naive Multinomial Bayes."
    (fun () ->
      ...
      let module NB = Naive_bayes.Binomial(
        struct
  ...
  add_simple_test ~title:"Naive Gaussian Bayes."
    (fun () ->
      ...
      let module GB = Naive_bayes.Gaussian(IrisEncoding) in
    ...
    (fun () ->
      ...
      let no_setosa =
        List.filter ~f:(fun (c,_) -> match c with | `setosa -> false | _ -> true)
          iris
      in
      let module LR = Logistic_regression.Binary(IrisEncoding) in
  ...
  add_simple_test ~title:"Logistic Regression on Iris all classes."
    (fun () ->
      let iris      = Iris.iris in
      let module LR = Logistic_regression.Multiclass(IrisEncoding) in
