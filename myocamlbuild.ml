          ...
          try
            ...
            let ic   =
              if not (Sys.file_exists file) then begin
                ignore (Sys.command (sprintf "mkdir -p %s" (Filename.dirname file)));
                ignore (Sys.command (sprintf "cp ../%s %s" file file));
              end;
